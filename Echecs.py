...
class Echecs(Jeu) : 
  ...
  def mouvements_autorises(self, etat, joueur) : 
    ...
    for (x,y) in etat.mouvements : 
      if (x,y) in liste_coups_possibles(self, joueur, etat) : 
        for (a,b) in liste_coups_possibles(self, joueur, etat)[x,y] : 
